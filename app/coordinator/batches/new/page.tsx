...
export default function NewBatchPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    division: '', studentCount: '', academicYear: '2024-2025',
  });
  ...
  return (
    <div className="flex items-start justify-center p-6 lg:p-10">
      <form onSubmit={handleSubmit} className="sm:mx-auto sm:max-w-5xl w-full">
        ...
        <h3 className="text-xl font-semibold text-foreground">Add New Batch</h3>
        <p className="mt-1 text-sm text-muted-foreground">Create a new student batch for timetable scheduling.</p>
        ...
        <div className="grid grid-cols-1 gap-10 md:grid-cols-3">
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-muted-foreground">Name and identifier for this student batch.</p>
          </div>
            ...
            <div className="space-y-4 md:space-y-6">
              <div>
                ...
                <div className="mt-2 flex gap-2">
                  <Input id="batchName" value={formData.name} required
                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                    placeholder="e.g., 3rd Year - Sem 5 - Div A" className="flex-1" />
                  <Button type="button" variant="outline" onClick={() => setFormData({ ...formData, name: generateName() })}>
                    Auto
                  </Button>
                </div>
                <p className="mt-2 text-sm text-muted-foreground">
                  Click &quot;Auto&quot; to generate from year, semester, and division.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 gap-10 md:grid-cols-3">
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-muted-foreground">Year, semester, division and student count.</p>
          </div>
          <div className="md:col-span-2">
            <div className="space-y-4 md:space-y-6">
        ...
        <div className="flex items-center justify-end space-x-4">
          ...
          <Button type="submit" isLoading={loading}>{loading ? 'Creating...' : 'Create Batch'}</Button>
        </div>
      </form>
    </div>
  );
